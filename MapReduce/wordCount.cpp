/*
	...
	Reads input files and stores words in a map where the key is
	the word, and the number of occurrences is the value.
 */
...
WordCount::MRmap(const std::map<std::string,std::string> &input,
	...
	for (auto it = input.begin(); it != input.end(); it++ ) {
		...
		do {
			std::string word;
			iss >> word;
			// Check each character in the word to see if it's a number, letter, or apostrophe.
